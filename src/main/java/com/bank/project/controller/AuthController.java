...
//    }
    ...
    public ResponseEntity<MessageResponse> ModifUser(@Valid @RequestBody User signUpRequest) {
        User userSaved;
        if (!userRepository.existsByUscode(String.valueOf(signUpRequest.getUscode()))) {
            userSaved=new User();
        }else{
        // Create new user's account
        Optional<User> user = userRepository.findByUscode(String.valueOf(signUpRequest.getUscode()));
         userSaved = user.get();
        }
...
//        userSaved.setEmail(signUpRequest.getEmail());
    ...
    public ResponseEntity<MessageResponse> deleteUser(@Valid @RequestBody SigndeleteRequest signUpRequest) {
        if (!userRepository.existsByUscode(signUpRequest.getUsername())) {
            return ResponseEntity
                    .badRequest()
...
//        }
        ...
        Optional<User> user = userRepository.findByUscode(signUpRequest.getUsername());
